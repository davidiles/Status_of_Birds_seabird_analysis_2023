...
spp_vec <- spp_vec[spp_vec %!in% c("BLOY")]

# Empty dataframes to store results
allspecies_percent_change_summary = allspecies_N_summary_regional = allspecies_regional_change_samples = data.frame()
...
for (spp in spp_vec){
  ...
  # ------------------------------------------------
  # Re-weight "plot-based" estimates based on the most recent colony-level abundance estimate
  # ------------------------------------------------
  plot_based_colonies <- unique(spdat_pcount$Colony_Name)
  
  # Read in 'total colony abundance' estimates
  pabund = read_xlsx("data/PACIFIC_DATA_2022.xlsx", sheet = 3) %>%
    subset(Species == spp) %>%
  ...
  for (colony in plot_based_colonies){
    ...
    N_est <- pabund$Total_Size_of_Colony[which(pabund$Colony_Name == colony)]
  ...
  # Dynamics from 1984 onwards
  colony_plot_1984 = ggplot() +
    geom_ribbon(data = subset(N_summary_colony, Year >= 1984), aes(x = Year, ymin = q05, ymax = q95), fill = "gray85", col = "transparent")+
    geom_line(data = subset(N_summary_colony, Year >= 1984), aes(x = Year, y = q50), col = "black")+
    geom_vline(data = subset(spdat, Count == 0 & Year >= 1984),aes(xintercept = Year), col = "red", size = 1.5, alpha = 0.3)+
    geom_point(data = subset(spdat, Species == spp & Year >= 1984),aes(x = Year, y = Count))+
    xlab("Year")+
  ...
  print(colony_plot_1984)
  
  pdf(paste0("output/Pacific/model_results/figures/",spp,"_colony_1984.pdf"), width = 20,height = 10)
  print(colony_plot_1984)
  dev.off()
  ...
  colonies_to_include_for_regional = colonies_to_include # %>% subset(first_survey <= 1990 & last_survey >= 2000)
  ...
  # Dynamics from 1984 onwards
  regional_plot_1984 = ggplot() +
    geom_ribbon(data = subset(N_summary_regional, Year >= 1984), aes(x = Year, ymin = q05, ymax = q95), fill = "gray85", col = "transparent")+
    geom_line(data = subset(N_summary_regional, Year >= 1984), aes(x = Year, y = q50), col = "black")+
    xlab("Year")+
    ylab("Population index")+
    scale_y_continuous(labels = comma)+
    ggtitle(paste0(spp," regional trajectory since 1984"))
  
  pdf(paste0("output/Pacific/model_results/figures/",spp,"_regional_1984.pdf"))
  print(regional_plot_1984)
  dev.off()
  ...
  for (baseline_year in c(1984)){
    ...
    pdf(paste0("output/Pacific/model_results/figures/",spp,"_percentchange_since_",baseline_year,".pdf"))
    print(spp_pchange_plot)
    dev.off()
...
# ------------------------------------------------
write.csv(allspecies_percent_change_summary, file = "output/Pacific/model_results/tables/Pacific_change_estimates_2022.csv", row.names = FALSE)

# ------------------------------------------------
# Output annual regional indices
# ------------------------------------------------
write.csv(allspecies_N_summary_regional, file = "output/Pacific/model_results/tables/Pacific_annual_indices_2022.csv", row.names = FALSE)

# ------------------------------------------------
# Generate a figure of the trajectory of individual species groups
# (in terms of percent change since 1984)
# ------------------------------------------------
...
pdf(paste0("output/Pacific/model_results/figures/0_status_",byear,".pdf"), width = 5, height = 5)
print(plot_status)
dev.off()
...
pdf(paste0("output/Pacific/model_results/figures/0_percent_change_allspecies_",byear,".pdf"), width = 7, height = 5)
print(plot_allspecies)
dev.off()
